...
public abstract class Opener {
    ...
    public Winning getWinning(Crate crate) {
        ...
        if (crate.getTotalPercentage() > 0) {
            ...
            winning = crate.getWinnings().get(CratesPlus.getOpenHandler().getCratesPlus().getCrateHandler().randInt(0,
                    crate.getWinnings().size() - 1));
        }
        return winning;
    }
    ...
    protected void finish(final Player player) {
        Bukkit.getScheduler().runTask(getPlugin(), (Runnable) () -> CratesPlus.getOpenHandler().getCratesPlus()
                .getCrateHandler().removeOpening(player.getUniqueId()));
    }
